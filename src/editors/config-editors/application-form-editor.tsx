...
export function ApplicationFormEditor({
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="mb-1 block text-xs font-medium text-zinc-500 dark:text-zinc-400">Programme Summary</label>
        <textarea value={c.programme_summary ?? ""} onChange={(e) => onChange({ ...config, programme_summary: e.target.value })} rows={2} className="w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-600 dark:bg-surface-base dark:text-white" />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-zinc-500 dark:text-zinc-400">Pricing Display</label>
        <textarea value={c.pricing_display ?? ""} onChange={(e) => onChange({ ...config, pricing_display: e.target.value })} rows={2} className="w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-600 dark:bg-surface-base dark:text-white" />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-zinc-500 dark:text-zinc-400">Submit Button Label</label>
        <input value={c.submit_button_label ?? ""} onChange={(e) => onChange({ ...config, submit_button_label: e.target.value })} className="w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-600 dark:bg-surface-base dark:text-white" placeholder="Apply Now" />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-zinc-500 dark:text-zinc-400">Post-Submit Message</label>
